...
function openChatBot() {
    ...
    sideBar.innerHTML = closesidebarIcon;
    ...
    // const headerSidebarButton = document.querySelector('.header-left .sidebar');
    const headerSidebarButton = document.querySelector('.header-left');




    // Add media query to handle mobile view
    function handleMobileView() {
        const isMobile = window.matchMedia("(max-width: 576px)").matches;
        const chatCloseButton = document.querySelector('.chat-close');
        if (isMobile) {
            // Hide close button from chat header if it exists
            if (chatCloseButton) {
            ...
            closeSidebarButton.style.display = 'inline-block';
        } else {
            // Show close button in chat header if it exists
            if (chatCloseButton) {
                chatCloseButton.style.display = 'inline-block';
            }
            // Show header sidebar button
            if (headerSidebarButton) {
                headerSidebarButton.style.display = 'inline-block';
            }
            ...
            // Hide sidebar close button (since we have the toggle in header)
    ...
    newChatButton.addEventListener('click', async function () {
        ...
        try {
            ...
            if (sessions.length >= 3) {
                // alert("You can only have up to 3 chat sessions.");
                console.log("You can only have up to 3 chat sessions.");
                return;
            }
    ...
    sideBar.addEventListener('click', function () {
        ...
        const rightColumn = document.querySelector('.chat-right-column');
        const isHidden = leftColumn.style.display === 'none';
    ...
    async function exportAsPdf() {
        ...
        function addHeader() {
            ...
            pdf.setFontSize(12);
            pdf.text("SitePace IntelliView™ – Virtual Tour Chat Transcript", margin, y);
            y += lineHeight;

            pdf.setFontSize(10);
            pdf.text(`Project: ${projectName}`, margin, y); y += lineHeight;
            pdf.text(`Date & Time of Export: ${exportTime}`, margin, y); y += lineHeight;
            pdf.text(`Generated by: SitePace IntelliView™ (AI Assistant)`, margin, y); y += lineHeight;
        ...
        function addFooter(pageNum, totalPages) {
            ...
            pdf.text("Generated using SitePace.ai's proprietary IntelliView™ platform.", margin, footerY);
...
<path d="M15.5845 0.805664L13.5048 2.88535L19.0907 8.47129L21.1704 6.3916C22.2446 5.31738 22.2446 3.57715 21.1704 2.50293L19.4774 0.805664C18.4032 -0.268555 16.663 -0.268555 15.5888 0.805664H15.5845ZM12.5337 3.85644L2.51767 13.8768C2.0708 14.3236 1.74423 14.8779 1.56376 15.4838L0.0426709 20.6529C-0.0647509 21.0182 0.0340772 21.4092 0.300483 21.6756C0.56689 21.942 0.957905 22.0408 1.31884 21.9377L6.48798 20.4166C7.09384 20.2361 7.64814 19.9096 8.09502 19.4627L18.1196 9.44238L12.5337 3.85644Z" fill="black"/>
        ...
        deleteOption.addEventListener('click', async function (e) {
            ...
            if (allSessions.length <= 1) {
                // alert("You cannot delete the last remaining chat session.");
                console.log("You cannot delete the last remaining chat session.");
                dropdownMenu.style.display = 'none';
                return;
            }
            ...
            if (success) {
                ...
                if (updatedSessions.length > 0) {
                    const latestSession = updatedSessions[updatedSessions.length - 1]; // Last = latest
                    currentSessionId = latestSession.id;

                    // Refresh sidebar with active session
                    await updateSidebarSessions(captureId);
                    clearChatMessages();

                    // Load chat history of latest session
...
function getRandomPrompts(arr, count = 4) {
